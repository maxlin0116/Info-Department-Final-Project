...
interface Snapshot {
  generatedAt: string;
  reservations: Array<{ id: string; area: string; title: string; status: string; startTime: string; endTime: string; participantCount: number }>;
...
export function PublicDisplay() {
  ...
  return <main className="min-h-screen bg-[#020617] text-slate-100 p-6 lg:p-8 overflow-hidden">
    ...
    <div className="grid xl:grid-cols-[1.05fr_1fr_1fr] gap-5">
      <section className="display-panel"><div className="display-panel-title"><Users />今日空間預約</div><div className="space-y-3 overflow-y-auto max-h-[70vh]">{snapshot?.reservations.length ? snapshot.reservations.map((item) => { const active = new Date(item.startTime) <= now && new Date(item.endTime) > now; return <div key={item.id} className={`rounded-xl border p-4 ${active ? "border-emerald-500/50 bg-emerald-500/10" : "border-slate-700 bg-slate-950/50"}`}><div className="flex justify-between gap-4"><div><div className="text-xs font-mono text-slate-500">{item.area}</div><h3 className="text-lg font-semibold mt-1">{item.title}</h3></div><span className={`text-xs font-mono ${item.status === "approved" ? "text-emerald-300" : "text-amber-300"}`}>{active ? "IN USE" : item.status.toUpperCase()}</span></div><div className="mt-3 text-slate-300 font-mono">{timeOnly(item.startTime)}–{timeOnly(item.endTime)} · {item.participantCount} 人</div></div>; }) : <Empty />}</div></section>
