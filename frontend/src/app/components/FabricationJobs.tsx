...
export function FabricationJobs() {
  ...
  return <div className="space-y-6">
    ...
    {loading ? <Loader2 className="animate-spin text-slate-500" /> : jobs.length === 0 ? <div className="p-10 text-center border border-dashed border-slate-700 rounded-xl text-slate-500">尚無加工工作</div> : <div className="grid gap-4">{jobs.map((job) => <article key={job.id} className="rounded-xl border border-slate-800 bg-slate-900/40 p-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4 text-sm"><div><span className="text-slate-500">預估</span><div className="text-slate-200">{formatMinutes(job.estimatedMinutes)}</div></div><div><span className="text-slate-500">顏色</span><div className="text-slate-200">{job.assignedColor || job.requestedColor || "—"}</div></div><div><span className="text-slate-500">開始</span><div className="text-slate-200">{formatDateTime(job.estimatedStartAt || job.startedAt)}</div></div><div><span className="text-slate-500">完成</span><div className="text-slate-200">{formatDateTime(job.expectedEndAt)}</div></div></div>
